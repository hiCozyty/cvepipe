...
variable "vm_name" {
  type    = string
  default = "win11-21h2-x64-enterprise-template"
}
...
locals {
  template_description = "Windows 11 21H2 64-bit Enterprise template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => localuser:password"
}

source "proxmox-iso" "win11-21h2-x64-enterprise" {
...
build {
  sources = ["source.proxmox-iso.win11-21h2-x64-enterprise"]
